...
def spmv_gpu_csr_benchmark(sp_matrix, t):
    ...
    # first run
    cuda_csr_spmv[nblocks, nthreads, stream](bf_csr_rowptr, bf_csr_colidx,
    ...
    for _ in range(t):
        ...
        cuda_csr_spmv[nblocks, nthreads, stream](bf_csr_rowptr, bf_csr_colidx,
                                                 bf_csr_val, bf_x, bf_csr_y)
        bf_csr_y.copy_to_host(csr_y, stream=stream)
        stream.synchronize()
        end = time.perf_counter()
        csr_time_list.append((end - start))
...
def spmv_gpu_sell_benchmark(sp_matrix, slice_height, t):
    ...
    # first run
    cuda_sell_spmv[nblocks, nthreads, stream](bf_sell_sliceptr,
                                              bf_sell_colidx,
                                              bf_sell_val, bf_x,
                                              slice_height,
                                              bf_sell_y)
    ...
    for _ in range(t):
        ...
        cuda_sell_spmv[nblocks, nthreads, stream](bf_sell_sliceptr,
                                                  ...
                                                  slice_height,
                                                  bf_sell_y)
        bf_sell_y.copy_to_host(sell_y, stream=stream)
        stream.synchronize()
        end = time.perf_counter()
        sell_time_list.append((end - start))
